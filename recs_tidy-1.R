# R by Example: Analyzing Residential Energy Consumption Survey 
#
# Author: Morteza Taiebat - University of Michigan 
# In this script, we use the 2015 RECS data to examine which US census region
...
avg_temp_by_type_region %>%
  ...
  ggplot( aes(y = avg_temp, x = region, color = `Winter Temperature`) ) +
    ...
    geom_errorbar( aes(ymin = lwr, ymax = upr), 
                   width = .3, position = position_dodge(width = 0.2)
    ) +
    theme_bw() +
